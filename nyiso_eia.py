...
def read_csv_auto(path: str) -> pd.DataFrame:
    ...
    if enc is None or enc.lower() in ["ascii", "utf-8", "utf_8"]:
...
def make_nyiso_power_temp_solar(
    ...
    final_power = df_sample[["Date", "Hour", "NYC", "NJ", "CT"]].reset_index(drop=True)
    final_power.to_csv(os.path.join(out_dir, "nyiso_power.csv"), index=False, encoding=encoding)
    ...
    temp_data = 28 + daily_temp + np.random.normal(0, 1, n)
    df_temp = pd.DataFrame({"일시": dates, "기온(°C)": temp_data.round(1)})
    df_temp.to_csv(os.path.join(out_dir, "temp.csv"), index=False, encoding=encoding)
    ...
    df_solar.to_csv(os.path.join(out_dir, "solar.csv"), index=False, encoding=encoding)

    return final_power, df_temp, df_solar


def merge_power_temp_solar(
    out_dir: str = ".",
    power_filename: str = "nyiso_power.csv",
    temp_filename: str = "temp.csv",
    solar_filename: str = "solar.csv",
    out_filename: str = "integrated_merged.csv",
    out_encoding: str = "utf-8-sig",
) -> pd.DataFrame:
    # 1) 파일 로드
    power_path = os.path.join(out_dir, power_filename)
    temp_path  = os.path.join(out_dir, temp_filename)
    solar_path = os.path.join(out_dir, solar_filename)

    power = read_csv_auto(power_path)
    temp  = read_csv_auto(temp_path)
    solar = read_csv_auto(solar_path)
    ...
    # 4) 저장 (최종 산출물)
    merged_path = os.path.join(out_dir, out_filename)
    merged.to_csv(merged_path, index=False, encoding=out_encoding)
...
def run_on_import(
    ...
    # 1) 원본 3개 CSV 생성
    make_nyiso_power_temp_solar(
        url=url, start=start, end=end, out_dir=out_dir, encoding=encoding, seed=seed
    )
    # 2) 생성된 3개 CSV -> merged 파일 생성
    merged = merge_power_temp_solar(out_dir=out_dir, out_filename="integrated_merged.csv")
    return merged
...
if _AUTORUN and not globals().get("_NYISO_EIA_ALREADY_RAN", False):
    ...
    try:
        run_on_import(out_dir=out_dir_used)
        print(f"✅ nyiso_eia import 완료: integrated_merged.csv 생성됨 (저장폴더: {out_dir_used})")
    except Exception as e:
        print(f"⚠️ nyiso_eia import 자동 실행 실패: {e}")
